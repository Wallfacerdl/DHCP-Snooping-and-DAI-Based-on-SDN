...
class SimpleDhcpSnooping(app_manager.RyuApp):
    ...
    def packet_in_handler(self, ev):
        ...
        if eth:
            ...
            self.logger.info("⏰ %s：📦 控制器从交换机收到数据包 - 端口: %s, 源MAC: %s, 目的MAC: %s, 以太网类型: 0x%04x", 
                        time_string,in_port, eth.src, eth.dst, eth.ethertype)
        
        # 检查IPv4报文
        ipv4_pkt = pkt.get_protocol(ipv4.ipv4)
        if ipv4_pkt:
            self.logger.info("  🌐 IPv4报文 - 协议: %d, 源IP: %s, 目的IP: %s", 
                        ipv4_pkt.proto, ipv4_pkt.src, ipv4_pkt.dst)
        
        # 检查UDP报文
        udp_pkt = pkt.get_protocol(udp.udp)
        if udp_pkt:
            self.logger.info("      📨 UDP报文 - 源端口: %d, 目的端口: %d", 
    ...
    def handle_dhcp_packet(self, datapath, in_port, dhcp_type, msg):
        ...
        if dhcp_type in ["DHCPOFFER", "DHCPACK"]:
            if in_port in self.trusted_ports:
                ...
                self.logger.warning("               🚫 拦截！非信任端口 %s 的DHCP响应", in_port)
                # 丢弃报文，不进行任何操作
        else:
    ...
    def get_dhcp_message_type(self, dhcp_pkt):
        """获取DHCP报文类型"""
        self.logger.info("          🔧 开始解析DHCP选项，选项数量: %d", len(dhcp_pkt.options.option_list))
